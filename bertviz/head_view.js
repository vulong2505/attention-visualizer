...
requirejs(["jquery", "d3"], function ($, d3) {
  ...
  function renderText(svg, text, isLeft, attention, leftPos) {
    ...
    const attentionValueDisplay = d3
      .select(`#${config.rootDivId}`)
      .append("div")
      .attr("class", "attention-value-display")
    ...
    tokenContainer.on("mouseover", function (d, index) {
      ...
      const attentionValues = isLeft
        ? attention.map((head) => head[index]) // Left token: attention[index][other_tokens]
        : attention.map((head) => head.map((row) => row[index])); // Right token: attention[other_tokens][index]

      const formattedAttention = attentionValues
        .map((head, headIndex) => {
          if (!config.headVis[headIndex]) return null; // Skip inactive heads
          return head.map((value, tokenIndex) => {
            const tokenPair = isLeft
              ? `${text[tokenIndex]} -> ${text[index]}`
              : `${text[index]} -> ${text[tokenIndex]}`;
            return `Head ${headIndex}, ${tokenPair}: ${value.toFixed(3)}`;
          });
        })
        .filter((d) => d) // Remove null entries for inactive heads
        .flat()
        .join("<br>");

      attentionValueDisplay
        .style("visibility", "visible")
        .style("left", `${d3.event.pageX + 10}px`)
        .style("top", `${d3.event.pageY + 10}px`)
        .html(formattedAttention);
    });
    ...
    attentionValueDisplay
      .style("visibility", "hidden") // Hides the tooltip
      .html("");
  }
